...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // if there's no access token, we'll be shown the default state.currentPage of AppPage.Welcome
    if (!accessToken) {
      actions.stopLoading();
      return;
    }
    ...
    // Remove old token
    // if token is within 10 seconds we'll prevent them from connecting (poll will almost be over)
    // since token duration and poll duration are approximately the same
  ...
  useEffect(() => {
    ...
    // if web socket connected and user doesnt belong to current poll
    if (
      myID &&
